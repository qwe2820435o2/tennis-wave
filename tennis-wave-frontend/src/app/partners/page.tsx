...
export default function PartnersPage() {
    const [searchResult, setSearchResult] = useState<UserSearchResultDto | null>(null);
    ...
    async function fetchPartners() {
        ...
        try {
            const result = await userService.getRecommendedPartnersWithPagination(currentPage, pageSize);
            ...
            <Pagination className="mt-8">
                <PaginationContent>
                    ...
                    {endPage < searchResult.totalPages && (
                        <>
                            {endPage < searchResult.totalPages - 1 && (
                                <PaginationItem>
                                    <PaginationEllipsis />
                                </PaginationItem>
                            )}
                            <PaginationItem>
                                <PaginationLink 
                                    ...
                                    onClick={(e) => {
                                        e.preventDefault();
                                        handlePageChange(searchResult.totalPages);
                                    }}
                                >
        ...
        <div className="min-h-screen flex flex-col items-center bg-gradient-to-br from-green-50 via-white to-blue-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 p-4">
            <div className="w-full max-w-2xl">
                <Card className="border-2 border-green-600 dark:border-green-500 rounded-2xl shadow-lg mt-8 p-8 bg-white dark:bg-gray-900">
                    <CardHeader>
                        <CardTitle className="text-2xl text-center text-green-700 dark:text-green-400 font-bold">
                            Recommended Partners
                        </CardTitle>
                        {searchResult && (
                            <p className="text-center text-sm text-gray-600 dark:text-gray-300">
                                Showing {partners.length} of {searchResult.totalCount} partners
                                {searchResult.totalPages > 1 && ` (page ${currentPage} of ${searchResult.totalPages})`}
                            </p>
                        )}
                    </CardHeader>
                    <CardContent>
